...
const LeftNavBar = (value: {categorias: Categoria[]}) => {
    const [categorias] = useState<Categoria[]>(value.categorias);
...
const ItensLojaFormatoCards = function() {
    const [itensLoja] = useState<Item[]>([]);
    const [categorias] = useState<Categoria[]>([]);
    const [renderMode] = useState('page-mode'); //useState('tuple-mode'); //useState('card-mode');
                        ...
                        <span className={'item-loja-span ' + renderMode}>
                            ...
                            <div className={'item-loja-tags-minibox ' + renderMode}>
                                {i.subcategorias?.map((sc:Subcategoria)=>
                                    <>
                                        <span className={'item-loja-tag ' + renderMode + ' ' + 'sc.cssClass'} style={{backgroundColor: getCor(sc.idCategoria)}}><span className={'item-loja-tag-text ' + renderMode}>{sc.nome}</span></span>
                                    </>
                                )}
                            </div>
                        </span>
